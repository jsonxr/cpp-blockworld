...
struct GLProgramResource {
 public:
  ...
  [[nodiscard]] auto get() const -> GLuint { return handle_; };
...
class GLProgram {
 public:
  ...
  [[nodiscard]] auto handle() const -> GLuint { return handle_.get(); };
  ...
  static auto create(const std::string &vertex, const std::string &fragment)
      -> GLProgram;
